...
Polygon::Polygon(const int _seed, const std::list<Polygon_Vertex*> & _vertices, const std::list<Polygon_Edge*> & _edges)
{
	for (std::list<Polygon_Vertex *>::const_iterator it_v = _vertices.begin() ; it_v != _vertices.end() ; it_v++) {
		...
		(*it_v)->set_as_member_of_polygon(this);
	}
	...
	std::list<Polygon_Vertex *> & vertices_zero)
{
	...
	if (t == 0) {
		for (std::list<Polygon_Vertex *>::iterator it_v = vertices.begin() ; it_v != vertices.end() ; it_v++) {
			...
			vertices_signs[v->id_object] = I->sign(v->M);
		}
	} else {
		for (std::list<Polygon_Vertex *>::iterator it_v = vertices.begin() ; it_v != vertices.end() ; it_v++) {
			Polygon_Vertex* v = (*it_v);
...
void Polygon::split_unconstrained_vertex(Intersection_Line* I, Polygon_Vertex* v, const FT & t, const CGAL_Point_2 & V_t,
	Polygon_Vertex* & v1, Polygon_Vertex* & v2, Constraint & C)
{
	// Part 1.
...
Polygon* Polygon::build_as_edge_propagates(Polygon_Vertex* v1, Polygon_Vertex* v2, Polygon_Vertex* v1_ts, Polygon_Vertex* v2_ts, Polygon_Edge* e, const FT & t,
	const int seed, const Constraint & C_ts, Polygon_Vertex* & v1_os, Polygon_Vertex* & v2_os)
{
	...
	if (C_1.first == nullptr) {
		v1_os = new Polygon_Vertex(v1_ts, SCHEDULE);
	} else {
		v1_os = new Polygon_Vertex(id, t, v1_ts->M, C_1, C_os);
	}

	if (C_2.first == nullptr) {
		v2_os = new Polygon_Vertex(v2_ts, SCHEDULE);
	} else {
		v2_os = new Polygon_Vertex(id, t, v2_ts->M, C_2, C_os);
	}
...
void Polygon::remove_intersectant_edge(Intersection_Line* I, Polygon_Vertex* v1, Polygon_Vertex* v2, const FT & t,
	...
	Constraint & C_1, Constraint & C_2, Constraint & C_ts)
{
	...
	delete_object(v1, v2, vertices, false);
...
Polygon* Polygon::build_as_unconstrained_vertex_propagates(Polygon_Vertex* v, const FT & t, 
	const int polygon_seed, Polygon_Vertex* v1_ts, Polygon_Vertex* v2_ts, const Constraint & C_ts, 
	Polygon_Vertex* & v1_os, Polygon_Vertex* & v2_os)
{
...
void Polygon::remove_vertices_colliding_on_intersection_line(Intersection_Line* I, Polygon_Vertex* v, const FT & t, const CGAL_Point_2 & V_t,
	Polygon_Vertex* & vc_ts, CGAL_Vector_2 & u_ref)
{
...
Polygon_Vertex* Polygon::get_tied_vertex(Intersection_Line* I, Polygon_Vertex* v, const FT & t, const CGAL_Point_2 & V_t)
{
...
Polygon_Vertex* Polygon::get_tied_vertex(Intersection_Line* I, Polygon_Vertex* v, const FT & t, const CGAL_Point_2 & V_t, const CGAL_Vector_2 & u)
{
	...
	Polygon_Vertex* v_c = get_tied_vertex(I, v, t, V_t);
...
void Polygon::insert_unconstrained_vertex(Intersection_Line* I, Polygon_Vertex* v, const FT & t,
	Polygon_Vertex* vc_ts, Polygon_Vertex* v_c)
{
...
void Polygon::remove_vertices_colliding_in_dead_end(Polygon_Vertex* v_1, Polygon_Vertex* v_2, const FT & t, const CGAL_Point_2 & V_t)
{
	...
	Polygon_Vertex* v = new Polygon_Vertex(v_1->id_plane, t, V_t, C_1, C_2);
	
	delete_object(e_01, e_12, e_23, edges, true);
	delete_object(v_1, v_2, vertices, true);
...
void Polygon::redirect_constrained_vertex(Intersection_Line* I, Polygon_Vertex* v, const FT & t, const CGAL_Point_2 & V_t,
	Polygon_Vertex* & v_de, Polygon_Vertex* & v_ts, Constraint & C_ts)
{
...
void Polygon::redirect_constrained_vertex(Intersection_Line* I, Polygon_Vertex* v, Polygon_Edge* e, const FT & t, const CGAL_Point_2 & V_t,
	const std::vector<std::pair<CGAL_Point_2, CGAL_Vector_2> > & D, const std::vector<std::pair<Constraint, Constraint> > & C,
	const std::list<Intersection_Line*> & I_discarded, Polygon_Vertex* & v_ts)
{
...
Polygon* Polygon::build_as_constrained_vertex_propagates(Polygon_Vertex* v, const FT & t,
	const int seed, Polygon_Vertex* vd_ts, Polygon_Vertex* v_ts,
	const Constraint & C_os, const Constraint & C_v, Polygon_Vertex* & v_os)
{
	...
	v_os = new Polygon_Vertex(v_ts, SCHEDULE);
	//v_os = new Polygon_Vertex(pid, t, v_ts->M, v_ts->dM, C_os, I_v, SCHEDULE);
	
	Polygon_Vertex* vd_os = new Polygon_Vertex(pid, t, vd_ts->M, C_v, C_os);
...
Polygon* Polygon::build_as_constrained_vertex_propagates(Polygon_Vertex* v, const FT & t,
	const int seed, const CGAL_Point_2 & V_t, const int k, 
	const std::vector<std::pair<CGAL_Point_2, CGAL_Vector_2> > & D, const std::vector<std::pair<Constraint, Constraint> > & C,
	const std::list<Intersection_Line*> & I_discarded, Polygon_Vertex* & v_k, Polygon_Vertex* & v_l)
{
...
next:
	...
	Polygon_Vertex *e_v1 = e->v1, *e_v2 = e->v2, *f_v1 = f->v1, *f_v2 = f->v2;
	...
	/*if (e->is_adjacent_to_segment()) {
		return false;
	}*/

	Intersection_Line* I_e = e->constraint.first;
...
void Polygon::get_active_constrained_vertices(Intersection_Line* I_1, Intersection_Line* I_2, Polygon_Vertex* & v_1, Polygon_Vertex* & v_2)
{
	...
	for (std::list<Polygon_Vertex*>::iterator it_v = vertices.begin() ; it_v != vertices.end() ; it_v++) {
		Polygon_Vertex* v = (*it_v);
		if (v->is_active && !v->unconstrained()) {
...
void Polygon::shift_vertices(Polygon_Vertex* v1, Polygon_Vertex* v2, const CGAL_Point_2 & M, const FT & t)
{
