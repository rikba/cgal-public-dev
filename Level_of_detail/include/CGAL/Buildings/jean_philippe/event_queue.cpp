...
{
	queue = std::map<FT, std::list<Event_Vertex_Line*> >();
}
...
void Event_Queue::push(Event_Vertex_Line* e_vl)
{
	...
	std::pair<std::map<FT, std::list<Event_Vertex_Line*> >::iterator, bool> it_pushed;
	it_pushed = queue.insert(std::make_pair(e_vl->t_intersectant, std::list<Event_Vertex_Line*>(1, e_vl)));
...
void Event_Queue::push(Event_Vertex_Line* e_vl, const std::map<FT, std::list<Event_Vertex_Line*> >::iterator it_push)
{
...
void Event_Queue::erase(Event_Vertex_Line* e_vl)
{
	...
	std::map<FT, std::list<Event_Vertex_Line*> >::iterator it_loc = e_vl->queue_iterator;
...
void Event_Queue::get_simultaneous_events_for_this_vertex(const int intersectant, const FT & t_intersectant, std::list<Event_Vertex_Line*> & E_VL) 
{
	...
	std::map<FT, std::list<Event_Vertex_Line*> >::iterator it_queue_begin = queue.begin();
	if (it_queue_begin == queue.cend() || it_queue_begin->first != t_intersectant) {
		return;
	}
	...
	while (it_e != E.end()) {
		Event_Vertex_Line* e = (*it_e);
		if (e->intersectant == intersectant) {
			it_e = E.erase(it_e);
			e->is_queued = false;
