...
void Partition_Vertex_Octree::set_query(std::vector<double> & query, const int i, const int j) const
{
	...
	for (int k = 0 ; k <= 1 ; k++) {
		...
		switch (l) {
		case 0: query[0] = x_min - eps, query[1] = x_min + eps; break;
		case 1: query[0] = x_max - eps, query[1] = x_max + eps; break;
		case 2: query[2] = y_min - eps, query[3] = y_min + eps; break;
		case 3: query[2] = y_max - eps, query[3] = y_max + eps; break;
		case 4: query[4] = z_min - eps, query[5] = z_min + eps; break;
		case 5: query[4] = z_max - eps, query[5] = z_max + eps; break;
		}
	}
}
...
void Partition_Vertex_Octree::get_vertices_for_bounding_box_edge(const int i, const int j, std::list<Partition_Vertex *> & R) const
{
	R.clear();
