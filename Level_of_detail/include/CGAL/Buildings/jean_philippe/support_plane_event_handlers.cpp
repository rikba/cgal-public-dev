...
void Support_Plane::unconstrained_vertex_intersects_line(const std::list<Event_Vertex_Line*> & E, Polygon_Vertex* v, const CGAL_Point_2 & V_t)
{
	if (E.size() != 1) {
	...
	Intersection_Line* I = lines[e->intersected];
...
void Support_Plane::unconstrained_vertex_intersects_line_kth_time(const std::list<Event_Vertex_Line*> & E, Polygon_Vertex* v, const CGAL_Point_2 & V_t)
{
	if (E.size() != 1) {
		throw std::logic_error("Error : unconstrained vertex intersects several lines : not implemented yet.");
	}
...
void Support_Plane::constrained_vertices_intersect(const std::list<Event_Vertex_Line*> & E, Polygon_Vertex* v1, Polygon_Vertex* v2, const CGAL_Point_2 & V_t)
{
	...
	if (E.size() > 1) {
		...
		for (std::list<Event_Vertex_Line*>::const_iterator it_e = E.begin() ; it_e != E.end() ; it_e++) {
			I.push_back(lines[(*it_e)->intersected]);
		}
	...
	v1->stop(t);
	v2->stop(t);
	v1->stop_segments(I_2, t);
	v2->stop_segments(I_1, t);

	// Merge vertices
	Polygon* P_ts = v1->get_polygon();
	P_ts->remove_vertices_colliding_in_dead_end(v1, v2, t, V_t);
}
...
void Support_Plane::constrained_vertex_intersects_line(const std::list<Event_Vertex_Line*> & E, Polygon_Vertex* v, const CGAL_Point_2 & V_t)
{
	if (E.size() == 1) {
		constrained_vertex_intersects_single_line(E.front(), v, V_t);
	} else {
		throw std::logic_error("Error : constrained vertex intersects several lines : disabled case -> recompile.");
		// constrained_vertex_intersects_multiple_lines(E, v);
	}
}



void Support_Plane::constrained_vertex_intersects_single_line(Event_Vertex_Line* e, Polygon_Vertex* v, const CGAL_Point_2 & V_t)
{
#ifdef PRINT_EVENTS
	append(e, "C2");
#endif
	...
	if (propagation_continues_at_intersection(I, V_t, C_v_ts, t, S_2, seed)) {
		...
		init_2_uplets_of_unidirectional_segments_of_null_length(I, V_t, W_t, v_ts, v_os, C_v_ts, t);
		...
		// if (!polygon_set->exists(S_3, seed) && !I_v->exists_segment_that_includes(M, C_os, t)) {
			...
			init_1_uplets_of_unidirectional_segments_of_null_length(I_v, V_t, W_t, vl, C_os, t);
			...
			if (propagation_continues_at_intersection(I, V_t, C_v_os, t, S_4, seed)) {
				...
				if (vl_ts->id_object < vl_os->id_object) {
					Polygon_Vertex::set_paired_vertices(vl_ts, vl_os);
				} else {
					Polygon_Vertex::set_paired_vertices(vl_os, vl_ts);
				}
				P_4->shift_vertices(vb, vl_ts, M, t);

				init_2_uplets_of_unidirectional_segments_of_null_length(I, V_t, W_t, vl_os, vl_ts, C_v_os, t);
				init_1_uplets_of_unidirectional_segments_of_null_length(I_v, V_t, W_t, vb, C_ts, t);

			} else {
				init_1_uplets_of_unidirectional_segments_of_null_length(I, V_t, W_t, vl_os, C_v_os, t); // (H34)
			}
		...
		v->stop(t);
		v->stop_segments(I, t);
		init_1_uplets_of_unidirectional_segments_of_null_length(I, V_t, W_t, v_ts, C_v_ts, t);
		delete v;
	}
}
...
void Support_Plane::constrained_vertex_intersects_multiple_lines(const std::list<Event_Vertex_Line*> & E, Polygon_Vertex* v, const CGAL_Point_2 & V_t)
{
#ifdef PRINT_EVENTS
	append(E.front(), "C2 *");
#endif
...
bool Support_Plane::constrained_vertex_intersects_multiple_lines_ts(Polygon_Vertex* v, const FT t, const CGAL_Point_2 & V_t, const CGAL_Point_3 & W_t,
		const std::vector<Intersection_Line*> & I, const std::vector<std::pair<Constraint, Constraint> > & C)
{
	...
	Polygon_Vertex* v_1;
	P_init->redirect_constrained_vertex(I[1], v, e, t, V_t, D, C, I_discarded, v_1);
	init_1_uplets_of_unidirectional_segments_of_null_length(I[1], V_t, W_t, v_1, C_0_ts, t);
	...
	for (int k = 1 ; k < n ; k++) {
		...
		if (propagation_continues_at_intersection(I[k], V_t, C_limits_k, t, S, seed)) {
			...
			Polygon_Vertex *v_k, *v_l;
			Polygon* P = Polygon::build_as_constrained_vertex_propagates(v, t, seed, V_t, k, D, C, I_discarded, v_k, v_l);

			init_1_uplets_of_unidirectional_segments_of_null_length(I[k], V_t, W_t, v_k, C_0_ts, t);
			if (k + 1 != n) {
				// If k + 1 == n, v_l == v and the segment initially carried by it continues its way
				init_1_uplets_of_unidirectional_segments_of_null_length(I[k + 1], V_t, W_t, v_l, C_0_ts, t);
			}
...
void Support_Plane::constrained_vertex_intersects_multiple_lines_os(Polygon_Vertex* v, const FT t, const CGAL_Point_2 & V_t, const CGAL_Point_3 & W_t,
		const std::vector<Intersection_Line*> & I, const std::vector<std::pair<Constraint, Constraint> > & C)
{
	int n = int(I.size());
	const Constraint & C_0_ts = C[0].first, C_0_os = C[0].second;
	...
	for (int k = 0 ; k < n - 1 ; k++) {
		...
		Polygon_Vertex* v_k_curr, *v_k_next;
		P->get_active_constrained_vertices(I[k], I[k + 1], v_k_curr, v_k_next);
		P->shift_vertices(v_k_curr, v_k_next, V_t, t);

		init_1_uplets_of_unidirectional_segments_of_null_length(I[k], V_t, W_t, v_k_curr, (k == 0 ? C_1_os : C_0_os), t);
		init_1_uplets_of_unidirectional_segments_of_null_length(I[k + 1], V_t, W_t, v_k_next, C_0_os, t);
	...
	Polygon_Vertex* v_n, *v_0;
	P->get_active_constrained_vertices(I[n - 1], I[0], v_n, v_0);
	P->shift_vertices(v_n, v_0, V_t, t);

	init_1_uplets_of_unidirectional_segments_of_null_length(I[n - 1], V_t, W_t, v_n, C_0_os, t);
	init_1_uplets_of_unidirectional_segments_of_null_length(I[0], V_t, W_t, v_0, C_1_ts, t);
...
void Support_Plane::get_sorted_lines(Polygon_Vertex* v, const std::list<Event_Vertex_Line*> & E, std::vector<Intersection_Line*> & I)
{
	...
	for (std::list<Event_Vertex_Line*>::const_iterator it_e = E.begin() ; it_e != E.end() ; it_e++) {
		Intersection_Line* I_k = lines[(*it_e)->intersected];
...
void Support_Plane::get_sorted_constraints(Polygon_Vertex* v, const std::vector<Intersection_Line*> & I, std::vector<std::pair<Constraint, Constraint> > & C)
{
	C.reserve(I.size());
...
bool Support_Plane::edge_is_propagating_laterally(Intersection_Line* I, Intersection_Line* I_l, Polygon_Vertex* v, const FT t, const CGAL_Point_2 & V_t)
{
	...
	// First check : is there a segment on line I_l on C_os side ?
	// If so, returns false
	...
	Polygon_Vertex* v_os = P_os->get_tied_vertex(I_l, v, t, V_t);
...
void Support_Plane::edge_intersects_line(const std::list<Event_Vertex_Line*> & E_1, Polygon_Vertex* v1, Polygon_Vertex* v2, const CGAL_Point_2 & V_1)
{
#ifdef PRINT_EVENTS
	append(E_1.front(), "D");
#endif
	...
	Intersection_Line* I = lines[e_vl->intersected];
	const FT t = e_vl->t_intersectant;
	const CGAL_Point_2 V_2 = v2->pt(t);
	...
	if (propagate_laterally_1) {
		const CGAL_Point_3 W_1 = backproject(V_1);
		edge_propagates_laterally(I, v1, t, V_1, W_1);
	}

	if (propagate_laterally_2) {
		const CGAL_Point_3 W_2 = backproject(V_2);
		edge_propagates_laterally(I, v2, t, V_2, W_2);
	}
}
	...
	const CGAL_Point_2 & V_1, const CGAL_Point_3 & W_1, const CGAL_Point_2 & V_2, const CGAL_Point_3 & W_2, const bool propagate_frontally)
{
	...
	if (propagate_frontally) {
		...
		build_n_uplets_of_segments_as_edge_collides_line(I, v1, v2, V_1, W_1, V_2, W_2, v1_ts, v1_os, v2_ts, v2_os, t);
		...
		if (!v1->unconstrained()) v1->stop_segments(I, t);
		if (!v2->unconstrained()) v2->stop_segments(I, t);

		// Create segments that correspond to [v1_ts(t), v2_ts(t)]
		build_n_uplets_of_segments_as_edge_collides_line(I, v1, v2, V_1, W_1, V_2, W_2, v1_ts, nullptr, v2_ts, nullptr, t);
...
void Support_Plane::edge_propagates_laterally(Intersection_Line* I, Polygon_Vertex* v, const FT t, const CGAL_Point_2 & V_t, const CGAL_Point_3 & W_t)
{
	...
	Polygon_Vertex *v3_I, *v3_I_v;
	P_3->get_active_constrained_vertices(I, I_v, v3_I, v3_I_v);
	P_3->shift_vertices(v3_I, v3_I_v, V_t, t);

	init_1_uplets_of_unidirectional_segments_of_null_length(I_v, V_t, W_t, v3_I_v, C_os, t);
	init_1_uplets_of_unidirectional_segments_of_null_length(I, V_t, W_t, v3_I, C_v_os, t);
	...
	if (polygon_set->exists(S, seed)) {
		...
		if (v3_I->id_object < v4_I->id_object) {
			Polygon_Vertex::set_paired_vertices(v3_I, v4_I);
		} else {
			Polygon_Vertex::set_paired_vertices(v4_I, v3_I);
		}

		init_1_uplets_of_unidirectional_segments_of_null_length(I_v, V_t, W_t, v4_I_v, C_ts, t);
		init_1_uplets_of_unidirectional_segments_of_null_length(I, V_t, W_t, v4_I, C_v_os, t);
	}
