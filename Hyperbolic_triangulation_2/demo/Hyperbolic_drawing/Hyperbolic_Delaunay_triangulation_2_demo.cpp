...
void apply_unique_words(std::vector<Point>& points, Point input = Point(0, 0), double threshold = 10, int word_length = 4/*13*/)
{
  ...
  for(size_t i = 0; i < unique_words.size(); i++) {
    ...
    points[i] = Point(res.first, res.second);
  }
}

void apply_unique_words_G(std::vector<Point>& points, Point input = Point(0, 0), double threshold = 12, int word_length = 12/*13*/)
{
  static vector<OctagonMatrix> unique_words;
  static bool generated = false;
  if(generated == false) {
...
*/    
  ...
  //this->graphicsView->scale(1500, 1500);
  this->graphicsView->shear(700, 700);
...
MainWindow::processInput(CGAL::Object o)
{
  ...
  if(CGAL::assign(p, o)){
    ...
    // delete
      ...
      apply_unique_words/*_G*/(points, current_point_b, 6, 4);
      points.push_back(current_point_b);
      dt.clear();
      dt.insert(points.begin(), points.end());
