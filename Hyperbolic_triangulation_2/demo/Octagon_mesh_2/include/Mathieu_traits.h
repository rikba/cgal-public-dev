...
#ifndef CGAL_MATHIEU_TRAITS_H
#define CGAL_MATHIEU_TRAITS_H
...
namespace CGAL {
	...
	template < class R >
	class Mathieu_traits : public R {
	public:
		typedef Mathieu_traits<R> Self;
		...
		typedef typename R::Construct_circumcenter_2   Euclidian_construct_circumcenter_2;
		...
		typedef typename R::Construct_midpoint_2          Euclidian_Construct_midpoint_2;
		...
		typedef boost::variant<Line_Segment_2,Arc_2 > Hyp_Segment_2;
	...
	public:
		...
		class Segment_2 : public Line_Segment_2, Arc_2 
		{
		public:
			Segment_2(Point_2 p, Point_2 q) : Line_Segment_2(p,q)
			{}
			Segment_2(Circle_2 c,Point_2 p, Point_2 q): Arc_2(c,p,q)
			{}
			Point_2 getarc(int i) const
			{
				if (i==1) {
					return this->get<1>();
				}
		...
		class Construct_reflexion  
		{		
			typedef typename CGAL::Regular_triangulation_filtered_traits_2<R> Regular_geometric_traits_2;
			typedef typename Regular_geometric_traits_2::Construct_weighted_circumcenter_2 Construct_weighted_circumcenter_2;
			typedef typename Regular_geometric_traits_2::Weighted_point_2 Weighted_point_2;
			typedef typename Regular_geometric_traits_2::Bare_point Bare_point;
			typedef	typename Regular_geometric_traits_2::Construct_vector_2 Construct_vector_2;
			typedef	typename Regular_geometric_traits_2::Construct_scaled_vector_2 Construct_scaled_vector_2;
			typedef	typename Regular_geometric_traits_2::Construct_translated_point_2 Construct_translated_point_2;
			

		public:
			Construct_reflexion(const Circle_2& c): _unit_circle(c)
			{
			}
			...
			{					
				...
				if(Collinear_2()(p, q, _unit_circle.center())){
					//TODO :return euclidian reflexion
					// should not happen for me
				}
		...
		Construct_reflexion
		construct_reflexion_object() const
		{
			return Construct_reflexion(_unit_circle);
		}
		...
		class Construct_segment_2
		{
			...
			public:
				...
				Segment_2 operator()(const Point_2& p, const Point_2& q) const
				{
					typedef typename R::Collinear_2 Collinear_2;
					if(Collinear_2()(p, q, _unit_circle.center())){
					return Segment_2(Circle_2(p,1000001),p, q);
					}
			...
			//	assert(circle.has_on_boundary(p) && circle.has_on_boundary(q));
				...
				{
					return Segment_2(circle,p, q);
				}
				return Segment_2(circle,q, p);
		}
		...
		{
		public:
			Construct_midpoint_2(const Circle_2& c) : _unit_circle(c) {}
			
				Point_2 operator()(Point_2 p, Point_2 q)
			{
				typedef typename R::Collinear_2 Collinear_2;
				
				Point_2 euc_midpoint = Euclidian_Construct_midpoint_2()(p, q);
				
				FT x0 = euc_midpoint.x(), y0 = euc_midpoint.y();
				FT radius = Compute_squared_distance_2()(p,euc_midpoint);
				// a*alphaˆ2 + b*alpha + c = 0;
				FT a = x0*x0 + y0*y0;
				FT b = a - radius + _unit_circle.squared_radius();
				FT c = _unit_circle.squared_radius();
				FT D = b*b - 4*a*c;
				
				FT alpha = (b - CGAL::sqrt(to_double(D)))/(2*a);
				
				Point_2 midpoint(x0/alpha, y0/alpha);
				if((Compute_squared_distance_2()(euc_midpoint,_unit_circle.center())) < (Compute_squared_distance_2()(midpoint,_unit_circle.center()))) 
				{
					std::cout << "cercle du mauvais sens" << std::endl;
				}
				return midpoint;
				/*
				typedef typename R::Collinear_2 Collinear_2;
				if (Collinear_2()(p,q,_unit_circle.center())){
					return Euclidian_Construct_midpoint_2()(p,q);
				}
				Circle_2 support = construct_supporting_circle(p,q);
				Point_2 A = support.center();
				FT rA = support.squared_radius();
				Weighted_point_2 wp(p);
				Weighted_point_2 wq(q);
				Weighted_point_2 wo(_unit_circle.center(), _unit_circle.squared_radius());
				Bare_point B = Construct_weighted_circumcenter_2()(wp, wo, wq);
				FT rB = Compute_squared_distance_2()(p, B);
				if (rB > 1000000 or rA>1000000) {
					return Euclidian_Construct_midpoint_2()(p,q);
				}
				FT xa= A.x(), xb = B.x(), ya = A.y(), yb=B.y();
				FT a = 2*(xb-xa), b=2*(yb-ya), c=(xb-xa)*(xb-xa) - rB +rA;

				FT delta = 4*a*a*c*c - 4*(a*a+b*b)*(c*c-b*b*rA);
				if (delta < 0) return Euclidian_Construct_midpoint_2()(p,q);
				std::cout << a << "   " << b << "   " << c << "    " << delta << std::endl;

				FT xp = xa + (2*a*c-(CGAL::sqrt(to_double(delta))))/(2*(a*a+b*b));
				FT xq = xa + (2*a*c+(CGAL::sqrt(to_double(delta))))/(2*(a*a+b*b));

				FT yp,yq;
				if (b==0) {
					 yp = ya +b/2+(CGAL::sqrt(to_double(rB-(2*c-a*a)/(2*a)*(2*c-a*a)/(2*a))));
					 yq = ya +b/2-(CGAL::sqrt(to_double(rB-(2*c-a*a)/(2*a)*(2*c-a*a)/(2*a))));

				}else {
					 yp=ya+(c-a*(xp-xa))/b;
					 yq=ya+(c-a*(xq-xa))/b;
				}
				std::cout << "jusquelacava" << std::endl;

				if (xp*xp+yp*yp < _unit_circle.squared_radius()) {
					return Point_2(xp,yp);
				}
				if (xq*xq+yq*yq < _unit_circle.squared_radius()) {
					return Point_2(xq,yq);
				}
		...
		private:
			...
			Circle_2 construct_supporting_circle(Point_2 p, Point_2 q) const
			{
				...
				Point_2 middle = Euclidian_Construct_midpoint_2()(p, q);
		...
		public:
			...
			{        
				...
				{ 
					Point_2 aux_center = Euclidian_construct_circumcenter_2()(p,q,r);
		...
		{ return Construct_midpoint_2(_unit_circle); }
		...
		Mathieu_traits(const Mathieu_traits & other) : 
		_unit_circle(other._unit_circle)
		{}
		
/*		Mathieu_traits &operator=
		(const Mathieu_traits &)
		{ return *this;}
